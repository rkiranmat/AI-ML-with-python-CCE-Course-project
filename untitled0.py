...
ModelML=filtered_df[['hvac_S','Avg_SouthWing_exter_temp']]
...
x1=ModelML['Avg_SouthWing_exter_temp'].to_numpy()
...
r_sq = model.score(x1.reshape((-1, 1)), y1)
...
plt.scatter(y_pred, y1, color="black")
plt.plot(y1, y1, color="blue", linewidth=3)
plt.xticks(())
plt.yticks(())
plt.show()

y_pred1=0.1941*x1
plt.scatter(y_pred1, y1, color="black")
plt.plot(y1, y1, color="blue", linewidth=3)
plt.xticks(())
plt.yticks(())
plt.show()
...
# plt.plot(y1, y1, color='#58b970', label='Regression Line')
# plt.scatter(y1, 0.2783*x1.reshape((-1, 1)), c='#ef5423', label='data points')
...
diabetes_y_pred = regr.predict(x1.reshape((-1, 1)))

# The coefficients
print("Coefficients: \n", regr.coef_)
# The mean squared error
print("Mean squared error: %.2f" % mean_squared_error(y1, diabetes_y_pred))
# The coefficient of determination: 1 is perfect prediction
print("Coefficient of determination: %.2f" % r2_score(y1, diabetes_y_pred))
# Plot outputs
plt.scatter(diabetes_y_pred, y1, color="black")
plt.plot(y1, y1, color="blue", linewidth=3)
...
sns.lineplot(x=ModelML.index,y=diabetes_y_pred,label='model data2')
sns.lineplot(x=ModelML.index,y=y_pred1,label='model data1')
plt.xticks(())
plt.yticks(())
...
ModelML1=filtered_df1[['hvac_S','Avg_SouthWing_exter_temp']]
# Remove row based on stats (mean - 3* std) for all signals
...
# Splitting y valriables based on rtu
y1a= ModelML1['hvac_S'].to_numpy()
x1a=ModelML1['Avg_SouthWing_exter_temp'].to_numpy()

x1ap=x1a

sns.lineplot(x=ModelML1.index,y=y1a,label='real data')
sns.lineplot(x=ModelML1.index,y=-219.7805+3.3137*x1ap,label='model data2')
sns.lineplot(x=ModelML1.index,y=0.1941*x1ap,label='model data1')
plt.xticks(())
plt.yticks(())
#plt.labels('real data','model data')
plt.xticks(rotation=90)
